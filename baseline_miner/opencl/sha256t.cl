...
__kernel void scan_nonces(
    ...
    state3[6] = H6;
    state3[7] = H7;
    sha256_compress(state3, hash_block2);

    // Check if hash meets target
    // Load target to private memory to avoid address space issues on Apple OpenCL
    if (hash_le_target(state3, target[0], target[1], target[2], target[3],
        ...
        if (idx < max_results) {
            ...
            #pragma unroll
            for (int i = 0; i < 8; i++) {
                result_hashes[idx * 8 + i] = state3[i];
            }
        }
    }
}

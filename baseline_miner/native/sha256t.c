...
static void sha256t_bytes(const uint8_t *data, size_t len, uint8_t out[32]) {
    uint8_t hash1[32];
    uint8_t hash2[32];
    SHA256_CTX ctx;
...
static PyObject *py_sha256t(PyObject *self, PyObject *args) {
    Py_buffer view;
    uint8_t hash2[32];
    ...
    sha256t_bytes((const uint8_t *)view.buf, (size_t)view.len, hash2);
...
static PyObject *py_scan_hashes(PyObject *self, PyObject *args) {
    ...
    for (i = 0; i < count; ++i) {
        ...
        // Third hash: hash of 32-byte second-hash digest with padding
        uint32_t hash_block2[16];
        hash_block2[0] = state2[0];
        hash_block2[1] = state2[1];
        hash_block2[2] = state2[2];
        hash_block2[3] = state2[3];
        hash_block2[4] = state2[4];
        hash_block2[5] = state2[5];
        hash_block2[6] = state2[6];
        hash_block2[7] = state2[7];
        hash_block2[8] = 0x80000000U;
        hash_block2[9] = 0;
        hash_block2[10] = 0;
        hash_block2[11] = 0;
        hash_block2[12] = 0;
        hash_block2[13] = 0;
        hash_block2[14] = 0;
        hash_block2[15] = 0x00000100U; /* 32 bytes * 8 */

        sha256_init_state(state1);
        sha256_compress_fast_words(state1, hash_block2);

        if (sha256_hash_le_target_be_words(state1, target_words)) {
            sha256_state_to_bytes(state1, hash2);
...
static PyMethodDef methods[] = {
    {"sha256t", py_sha256t, METH_VARARGS, "Triple SHA-256 hash."},
    {"scan_hashes", py_scan_hashes, METH_VARARGS, "Scan nonces for SHA256t hashes below target."},
    {NULL, NULL, 0, NULL}
};
...
static struct PyModuleDef module = {
    ...
    "_sha256t",
    "Portable SHA256t backend.",
    -1,
    methods
};

PyMODINIT_FUNC PyInit__sha256t(void) {
